...
class BackupConverter {
  ...
  static Future<bool> importConvertedBackup(BuildContext context) async {
    try {
      ...
      await Future.forEach(convertedData.entries, (MapEntry<String, dynamic> entry) async {
        final key = entry.key;
  ...
  static Future<Map<String, dynamic>> _convertBackupData(Map<String, dynamic> data) async {
    ...
    try {
      ...
      if (data.containsKey('saved_albums') && data['saved_albums'] is List) {
        ...
        for (int i = 0; i < savedAlbums.length; i++) {
          try {
            ...
            try {
              ...
              Logging.severe("Failed to convert album directly, applying fixes: $e");
              
              // Apply fixes to common issues:
              if (!albumData.containsKey('collectionId') && albumData.containsKey('id')) {
                albumData['collectionId'] = albumData['id'];
                Logging.severe("Fixed missing collectionId using id");
              }
              
              if (!albumData.containsKey('artistName') && albumData.containsKey('artist')) {
                albumData['artistName'] = albumData['artist'];
                Logging.severe("Fixed missing artistName using artist");
              }
              
              if (!albumData.containsKey('collectionName') && albumData.containsKey('name')) {
        ...
        Logging.severe("Album conversion summary: $successCount succeeded, $failureCount failed");
        
        newData['saved_albums'] = newSavedAlbums;
        
        // Handle album order - either use existing order or create a new one
        if (!data.containsKey('saved_album_order') || (data['saved_album_order'] as List).isEmpty) {
          newData['saved_album_order'] = newAlbumIds;
          Logging.severe("Created new album order with ${newAlbumIds.length} albums");
        } else {
          // Keep original order when possible, but make sure all albums are included
          List<String> albumOrder = List<String>.from(data['saved_album_order']);
          ...
          Logging.severe("Preserved and extended album order to ${albumOrder.length} albums");
        }
      }
      ...
      if (data.containsKey('custom_lists') && data['custom_lists'] is List) {
        ...
        for (int i = 0; i < customLists.length; i++) {
          try {
            ...
            Logging.severe("Processed custom list: ${list['name']} with ${list['albumIds'].length} albums");
          } catch (e) {
          ...
          try {
            ...
            for (String ratingJson in ratings) {
              try {
                ...
                if (!rating.containsKey('position') && rating.containsKey('trackNumber')) {
  ...
  static Future<bool?> _showPreviewDialog(BuildContext context, Map<String, dynamic> data) async {
    int albumCount = 0;
    int listCount = 0;
